...
//       }
//     }
//     const res = secondHalf + firstHalf;
//     return shuffle(res, iter - 1);
//   }
//   return shuffle(str, ite);
// }
...
function getNearestBigger(number) {
  let searchValue = number + 1;
  function splitToDigitCollection(num) {
    let n = num;
    const arrOfDigits = [];
    while (n > 0) {
      const value = n % 10;
      arrOfDigits.push(value);
      n = Math.floor(n / 10);
    }
    return arrOfDigits.sort();
  }
  while (
    splitToDigitCollection(number).join() !==
    splitToDigitCollection(searchValue).join()
  ) {
    searchValue += 1;
  }
  return searchValue;
